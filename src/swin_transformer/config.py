...
def update_config(config, args):
    ...
    neq_dict = [
        "NEQ.GENERAL.SEED", 0,
        "NEQ.GENERAL.VALID_SIZE", 0.001,
        "NEQ.NEQ.EPS", 1e-8,
        "NEQ.NEQ.VELOCITY_MU", 0,
        "NEQ.SCHEDULER.PRESET_SCHEDULER", True,
        "NEQ.SCHEDULER.SCHEDULER_FACTOR", 10,
        "NEQ.SCHEDULER.SCHEDULER_PATIENCE", 10,
        "NEQ.SCHEDULER.SCHEDULER_THRES", 0,
        "NEQ.SCHEDULER.SCHEDULER_METRIC", "vloss",
        "NEQ.LOGGING.PROJECT_NAME", "NEq2-S",
        "NEQ.LOGGING.RUN_NAME", ""
    ]
    config.merge_from_list(neq_dict)
    config.NEQ.GENERAL.SEED = args.seed
    config.NEQ.GENERAL.VALID_SIZE = args.valid_size

    config.NEQ.NEQ.EPS = args.eps
    config.NEQ.NEQ.VELOCITY_MU = args.velocity_mu

    config.NEQ.SCHEDULER.PRESET_SCHEDULER = args.preset_scheduler
    config.NEQ.SCHEDULER.SCHEDULER_FACTOR = args.scheduler_factor
    config.NEQ.SCHEDULER.SCHEDULER_PATIENCE = args.patience
    config.NEQ.SCHEDULER.SCHEDULER_THRES = args.thres
    config.NEQ.SCHEDULER.SCHEDULER_METRIC = args.metric

    config.NEQ.LOGGING.PROJECT_NAME = args.project_name
    config.NEQ.LOGGING.RUN_NAME = args.run_name
    """
