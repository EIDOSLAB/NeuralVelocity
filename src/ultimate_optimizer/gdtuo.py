...
class RMSPropAlpha(Optimizable):
    ...
    def step(self, params):
        ...
        for name, param in params.items():
            ...
            g = param.grad.detach()
            self.cache[name]['s'] = s = self.gamma * self.cache[name]['s'].detach() + (1. - self.gamma) * torch.square(
                g)
            self.all_params_with_gradients.append(s)
            params[name] = param.detach() - alpha * g / torch.sqrt(s + self.eps)
...
class Adam(Optimizable):
    ...
    def step(self, params):
        ...
        for name, param in params.items():
            ...
            g = param.grad.detach()
            self.cache[name]['m'] = m = beta1 * self.cache[name]['m'].detach() + (1. - beta1) * g
            self.cache[name]['v'] = v = beta2 * self.cache[name]['v'].detach() + (1. - beta2) * g * g
...
class ModuleWrapper(Optimizable):
    ...
    def get_lr(self):
        return self.optimizer.parameters['alpha'].detach().cpu().item()
